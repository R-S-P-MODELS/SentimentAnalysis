...
CumulativeSentimentPart=function(a,block=80,linguagem,Livro){
  #library(janeaustenr)
  library(dplyr)
  library(stringr)
  library(tidytext)
  
  tidy_books <-data.frame(Livro,a,1:length(a))
  names(tidy_books)=c("book","word","linenumber")
  ...
  library(plotly)
  jane_austen_sentiment$sentiment=cumsum(jane_austen_sentiment$sentiment)
  jane_austen_sentiment$index=jane_austen_sentiment$index/max(jane_austen_sentiment$index)
...
LocalSentimentPart=function(a,block=80,linguagem,livro){
#library(janeaustenr)
library(dplyr)
library(stringr)
library(tidytext)
...
#jane_austen_sentiment$index<-jane_austen_sentiment$index/max(jane_austen_sentiment$index)
return(jane_austen_sentiment)
#p1<-geom_col(aes(jane_austen_sentiment$index, jane_austen_sentiment$sentiment,fill =  jane_austen_sentiment$book),show.legend = FALSE)
return(p1) 
#ggplotly(p1)
}
...
LocalEmotionsPart=function(a,block=80,linguagem,Livro){
  #library(janeaustenr)
  library(dplyr)
  library(stringr)
  library(tidytext)
  ...
  jane_austen_sentiment <- tidy_books %>%
    ...
    spread(sentiment, n, fill = 0) #%>%
  #mutate(sentiment = positive - negative)
  df1=data.frame(jane_austen_sentiment$index,jane_austen_sentiment$anger,paste("Anger_",Livro,sep="")  )
  df2=data.frame(jane_austen_sentiment$index,jane_austen_sentiment$anticipation,paste("Anticipation_",Livro,sep=""))
  df3=data.frame(jane_austen_sentiment$index,jane_austen_sentiment$disgust,paste("Disgust_",Livro,sep=""))
  df4=data.frame(jane_austen_sentiment$index,jane_austen_sentiment$fear,paste("Fear_",Livro,sep=""))
  df5=data.frame(jane_austen_sentiment$index,jane_austen_sentiment$joy,paste("Joy_",Livro,sep=""))
  df6=data.frame(jane_austen_sentiment$index,jane_austen_sentiment$sadness,paste("Sadness_",Livro,sep=""))
  df7=data.frame(jane_austen_sentiment$index,jane_austen_sentiment$surprise,paste("Surprise_",Livro,sep=""))
  df8=data.frame(jane_austen_sentiment$index,jane_austen_sentiment$trust,paste("Trust_",Livro,sep=""))
  names(df1)=names(df2)=names(df3)=names(df4)=names(df5)=names(df6)=names(df7)=names(df8)=c('Index','Emotion','Name')
  df1=rbind(df1,df2,df3,df4,df5,df6,df7,df8)
  library(plotly)
  #jane_austen_sentiment$sentiment=cumsum(jane_austen_sentiment$sentiment)
...
CumulativeEmotionsPart=function(a,block=80,linguagem,Livro){
  #library(janeaustenr)
  library(dplyr)
  library(stringr)
  library(tidytext)
  ...
  Normalizador<-max(jane_austen_sentiment$anger)+max(jane_austen_sentiment$anticipation)+max(jane_austen_sentiment$disgust)+max(jane_austen_sentiment$fear)+max(jane_austen_sentiment$joy)+max(jane_austen_sentiment$sadness)+max(jane_austen_sentiment$surprise)+max(jane_austen_sentiment$trust)
  jane_austen_sentiment$index=jane_austen_sentiment$index/max(jane_austen_sentiment$index)
  
  jane_austen_sentiment$anger=(jane_austen_sentiment$anger)/Normalizador
  jane_austen_sentiment$anticipation=(jane_austen_sentiment$anticipation)/Normalizador
  jane_austen_sentiment$disgust=(jane_austen_sentiment$disgust)/Normalizador
  jane_austen_sentiment$fear=(jane_austen_sentiment$fear)/Normalizador
  jane_austen_sentiment$joy=(jane_austen_sentiment$joy)/Normalizador
  jane_austen_sentiment$sadness=(jane_austen_sentiment$sadness)/Normalizador
  jane_austen_sentiment$surprise=(jane_austen_sentiment$surprise)/Normalizador
  jane_austen_sentiment$trust=(jane_austen_sentiment$trust)/Normalizador
